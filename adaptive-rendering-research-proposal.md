...
> **`stale` 셀의 실행 비용.** `revalidate` 창이 지나야 stale이 만들어지고, stale 응답 한 번이 백그라운드 재생성을 유발하므로 **반복마다 다시 기다려야 한다**(창이 60초면 반복당 ~62초). SSG 라우트 10종 × 80조건 × 30회 = 24,000회가 여기에 걸려 순수 대기만 400시간을 넘는다. 따라서 실행에서는 **SSG 라우트에 짧은 재검증 주기(예: 5초)를 쓰는 실험용 변형을 따로 두거나**, stale 축의 반복 수를 줄이고 그 사실을 기록해야 한다. 이는 §3.1.2의 $T$가 셀 정의에 함께 들어가야 함을 뜻한다.
