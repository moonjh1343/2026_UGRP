...
- **In-process edge inference only.** A single tree distilled from the LightGBM ensemble is evaluated inside Lambda@Edge. A SageMaker endpoint in the request path would add tens of ms and cancel out the improvement it is measuring. Target overhead: TTFB increase < 2 ms — the deployed tree measures p95 0.025 ms. The proposal's depth-5/~50KB figure held only at λ=1; the deployed tree is depth 12 / 687KB (see stage 7 above), still far under the origin-request 50MB limit.
