...
두 실패 모두 **검증이 통과시키지 않고 거부했다는 점**이 중요하다. 이 상태로 8,800셀 ×
30회를 수집했다면 264,000회를 버렸을 것이고, 그때는 이미 며칠이 지난 뒤다.
...
제안서 §5.2는 ISR 캐시 상태(miss/hit/stale)를 "SSG·ISR 셀에 한해 추가 축"으로 명시해 놓고,
조건 수는 그 축 없이 계산한다.

```
제안서 표기      4 × 5 × 4 × Σ_r|M(r)| = 80 × 110 = 8,800
캐시 축 반영     80 × (110 + 10×2)      = 80 × 130 = 10,400
```

SSG 가능 라우트가 10종(콘텐츠 5 + 목록 5)이고 각각 캐시 상태가 3개이므로, SSG 셀 10개가
30개로 늘어난다. 워커의 `expandGrid()`는 **후자로 구현**했다 — 캐시 상태를 통제하지 않으면
한 셀 안에서 미스와 히트가 섞여 분산이 폭발하고, 그것이 §3.1.2의 `missRate`를 관측
불가능하게 만들기 때문이다. 제안서의 8,800이라는 수를 유지하려면 캐시 축을 빼야 하는데,
그건 `missRate`를 포기하는 것이다. **제안서 쪽 수식을 고치는 편이 옳다.**
