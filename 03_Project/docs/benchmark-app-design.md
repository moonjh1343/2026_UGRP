...
30. **`stale` 셀은 반복당 ~62초가 든다.** 엔트리를 만든 뒤 revalidate 창(60초)이 지나야 하고, stale 응답 한 번이 백그라운드 재생성을 유발해 반복마다 다시 기다려야 한다. `revalidate`가 세그먼트 설정 리터럴이라 실험용으로 줄일 수 없는 것이 원인이다(§11-14와 같은 뿌리). 대량 수집 전에 **짧은 revalidate를 가진 SSG 라우트 계열을 따로 두는 편이 낫다.**
