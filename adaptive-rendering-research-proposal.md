...
- 행동 공간: $m \in M$ = {CSR, SSR, Streaming SSR, SSG/ISR, Islands}
- 비용 함수:

$$J(x, m) = \sum_{i} w_i \cdot z_r(\text{QoE}_i) + \lambda \cdot \text{ServerCost}(x, m)$$

여기서 $z_r(\cdot)$은 라우트 $r$ 기준 z-score 정규화, QoE는 {LCP, INP, TBT, TTFB}, ServerCost는 요청당 렌더링 CPU 시간(ms) + 메모리 점유이다.

- 정책: $\pi(x) = \arg\min_{m \in M} \hat{f}_\theta(x, m)$
...
| 그룹 | 특징 | 수집 경로 |
|---|---|---|
| **서버 상태** | CPU/메모리 사용률, 이벤트 루프 지연(p95), 동시 요청 수, 렌더 큐 깊이, 최근 60초 TTFB p95, 캐시 적중률 | 애플리케이션 메트릭 → 인메모리 공유 상태 |
...
| **라우트/콘텐츠** | 예상 DOM 노드 수, 라우트 JS 번들 크기, 인터랙티브 컴포넌트 수, 데이터 페치 depth, 개인화 여부, SEO 중요도 | **빌드 타임 정적 분석**으로 사전 산출 후 룩업 테이블화 |
...
| 장치 | 내용 |
|---|---|
| **하드 규칙 오버라이드** | 크롤러 UA, 결제·인증 라우트는 ML 판단과 무관하게 SSR 고정. SEO·정합성 리스크를 모델에 위임하지 않음 |
| **마진 기반 폴백** | 1·2위 예측 차이 < $\tau$ 이거나 앙상블 분산이 임계 초과 → 기본 모드(Streaming SSR) |
| **전환 빈도 상한** | 동일 세션 내 모드 전환은 세션당 1회로 제한, 세션 단위 결과 캐싱 |
...
$$L = L_{\text{MSE}} + \alpha \sum_{(m_i, m_j)} \max\big(0,\ \epsilon - (\hat{J}_{m_j} - \hat{J}_{m_i}) \cdot \mathrm{sign}(J_{m_j} - J_{m_i})\big)$$

$\alpha$는 검증셋의 pairwise accuracy를 기준으로 튜닝한다.
...
1. **Phase 1 — 오프라인 정책 고정:** 서로게이트 모델 argmin 정책을 고정 배포, 5% 탐색 트래픽만 무작위 배정
2. **Phase 2 — 컨텍스추얼 밴딧:** 서로게이트 예측을 사전(prior)으로 사용하는 LinUCB로 전환. 보상은 세션 종료 시 수집된 실측 $-J$

$$m_t = \arg\min_m \left( \hat{f}_\theta(x_t, m) - \beta \sqrt{x_t^\top A_m^{-1} x_t} \right)$$

밴딧 상태($A_m$, $b_m$)는 DynamoDB에 유지하며 스트림 컨슈머가 갱신한다.
...
조건 수 = 4 × 5 × 25 × 4 × 5 = **10,000 셀**, 셀당 30회 반복 → **300,000 측정**, 중앙값 집계 시 10,000 완전 관측.
...
| **비용 효율** | 요청당 렌더링 CPU 시간, 동일 SLA 하 필요 태스크 수 |
...
| **3개월** | 랩 전수 실험 실행, Device Farm 보정 실험 | 랩 데이터셋(10,000 셀) |
...
| 렌더링 모드 | 5 | CSR / SSR / Streaming SSR / SSG·ISR / Islands |

*주 실험은 서버 티어를 M으로 고정한 10,000셀 그리드로 진행하고, 서버 티어·워커 리전은 별도 축소 그리드(각 500~1,000셀)로 부가 분석한다. 전체 교차(500,000셀)는 비용·시간상 비현실적이므로, 부분요인 설계(fractional factorial)로 주효과와 주요 2차 교호작용만 추정한다.*
