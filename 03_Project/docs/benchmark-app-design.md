...
content만 이상치 제거율이 24%(36/150)다. dashboard 3%, form 1%와 대비된다.
MAD 기준 3σ는 ±75점인데 그보다 멀리 떨어지는 반복이 반복적으로 나온다는 뜻이고,
분포가 단봉이 아닐 가능성이 높다. content-03은 페이로드가 가장 크므로(111KB)
3G에서 전송 시간이 리소스 경계에 따라 갈라지는 것이 유력한 후보다.

**MAD 트리밍은 이것을 고치는 게 아니라 가린다.** 실제 수집 전에 원인을 규명해야 하며,
원인이 셀 정의의 누락(통제되지 않은 축)이라면 그 축을 그리드에 넣어야 한다.
