...
factorial 수집 시에도 배제된 셀은 **아예 측정하지 않는다.** 측정해서 나중에 거르는 게 아니라, 그리드 정의에서 빠진다. 10,400셀은 이 배제와 SSG의 캐시 축을 함께 반영한 뒤의 수다(§12).
...
├── workers/                           Playwright 측정 워커
│   ├── lib/grid.mjs                   조건 그리드 · 시드 PRNG (버전 관리되는 실험 정의)
│   ├── lib/interactions.mjs           유형별 결정적 상호작용 시퀀스 (INP 유발)
│   ├── lib/measure.mjs                한 셀 1회 — 스로틀링 · 캐시 상태 통제 · 비콘 조인
│   ├── lib/stats.mjs                  강건 통계 · MAD 이상치 제거 · 5단계 판정식
│   ├── lib/checkpoint.mjs             JSONL 체크포인트 (append-only)
│   ├── run.mjs                        factorial 수집 실행기 (재개 가능)
│   └── verify-variance.mjs            5단계 합격 기준 검사
├── load/                              배경 부하
│   ├── profile.json                   라우트 믹스 — k6와 로컬 생성기가 둘 다 읽는다
│   ├── background.js                  k6 스크립트 (배포 산출물)
│   ├── generator.mjs                  로컬 대체 생성기
│   └── calibrate.mjs                  목표 CPU → VU 이진 탐색
├── infra/                             AWS CDK
└── docs/                              이 문서
```

> **부하 생성기가 둘인 이유.** 배포는 k6(Fargate Spot)를 쓰지만 이 개발 환경에는 k6
> 바이너리가 없다. 두 생성기가 **같은 `profile.json`을 읽게** 한 것이 핵심이다 — 믹스
> 정의가 갈라지면 로컬에서 캘리브레이션한 VU 수를 배포로 옮길 수 없고, "부하 65%"라는
> 셀 이름이 두 환경에서 다른 것을 뜻하게 된다.
...
낭비다. 10,400셀 × 30회에 불필요한 추론이 얹히면 그 자체가 서버 부하 피처를 오염시킨다.
...
### 세션 전환 상한은 라우트 단위로 센다 — 확정

**(세션, 라우트) 쌍으로 센다.** 초기 제안서 §3.5는 "세션당 1회"라고 썼으나, 세션 전역으로
세면 첫 방문 라우트의 결정이 세션 내내 **모든** 라우트를 고정해 버린다. 한 세션이 여러
라우트를 방문하는 것은 정상이고 **라우트마다 최적 모드가 다르다는 것이 이 연구의 전제**이므로,
그 전제를 안전장치가 스스로 부정하는 꼴이 된다. 제안서 §3.5를 이 정의로 고쳤다.
...
19. **부하를 피드백 제어하지 말 것.** "CPU 65%"는 조건이지 명령이다. 측정 중에 CPU를 보고 VU를 조정하면 부하가 측정 요청에 **반응**하게 되어 외생성이 깨진다 — 렌더 모드가 서버 부하를 바꾸고 그 부하가 동시에 피처인데, 부하가 다시 모드에 반응하면 인과 방향이 닫힌 고리가 된다. 캘리브레이션은 측정 전에 끝내고 측정 중에는 VU를 얼린다.
20. **반복 × 모드를 인터리빙할 것.** 모드별로 30회씩 몰아 돌면 각 모드가 서로 다른 시간대를 점유해, 그 시간대의 인프라 상태(열 스로틀링, GC 리듬, 캐시 워밍)가 모드 효과로 잘못 귀속된다. 조건 순서 무작위화(제안서 §5.2)는 셀 사이뿐 아니라 **반복 사이에도** 적용된다.
21. **분산비만으로 5단계를 판정하지 말 것.** `pooled_sd / 전체_폭`은 CSR처럼 혼자 3배 떨어진 모드가 하나만 있어도 폭이 그것에 지배되어, 나머지 모드가 전혀 구분되지 않아도 통과한다. 정책이 하는 일은 argmin이므로 **순위 맨 위를 분해할 수 있는가**(분해능 = `pooled_sd / (2위−1위)`)를 함께 봐야 한다. 단 분해능은 합격 조건이 아니다 — 상위 두 모드가 실제로 동률인 것은 참인 발견이고, 그래서 τ가 존재한다.
22. **상호작용에 `fill()`·`selectOption()`을 쓰면 INP가 결측된다.** Playwright는 값을 CDP `Input.insertText`로 넣는데, 이는 사용자 상호작용으로 집계되지 않는다(`interactionId` 없음, `performance.interactionCount` 증가 없음). 5단계 스모크 테스트에서 폼형만 INP가 전부 null이었고, **하필 폼형의 지배 축이 INP다.** `click` + `pressSequentially`, select에는 `press('ArrowDown')`을 쓴다.
23. **가벼운 위젯에서는 web-vitals만으로 INP가 잡히지 않는다.** `event` 타이밍은 durationThreshold(web-vitals 기본 40ms) 미만이면 엔트리가 **아예 전달되지 않고**, 엔트리가 없으면 `reportAllChanges`도 부를 것이 없어 확정이 페이지 숨김까지 밀린다. 앱이 임계값 16ms로 직접 관측하고, 그보다 빠른 경우는 `interactionCount`로 "상호작용은 있었으나 임계값 미만"(INP=0)과 "상호작용이 아예 없었음"(결측)을 구분해야 한다 — 둘은 다른 사실이다.
24. **측정 워커는 로컬호스트만 때려도 외부 네트워크에 오염된다.** 5단계 재측정 한 번이 실행 중 NIC 단절로 통째로 망가졌다 — 값이 3~5배 부풀고 순위까지 뒤섞였으며 MAD가 두 자릿수에서 네 자릿수로 튀었다. 요청은 `127.0.0.1`로만 나가지만 **브라우저 프로세스는 그렇지 않다**: NIC가 끊기면 Chrome이 DNS·연결 재시도를 반복하고 그 CPU가 스로틀링된 렌더러와 같은 코어를 두고 경쟁한다. `networkidle` 대기도 함께 밀린다. 실배포에서 측정 워커를 격리된 서브넷·보안 그룹에 두는 것은 부하 생성기와의 분리 때문만이 아니라 이 때문이기도 하다.
25. **환경 오염은 두 가지 모양으로 온다 — 둘 다 봐야 한다.** 지속적 변화(열 스로틀링, 캐시 워밍)는 전반/후반 벽시계 중앙값 비교로 잡히지만, 간헐적 사건(네트워크 단절, 다른 프로세스의 순간 부하)은 중앙값을 거의 바꾸지 않으면서 분산만 폭발시킨다. **드리프트와 스파이크 비율을 함께 보고**해야 "노이즈가 크다"는 결론에서 원인이 앱인지 환경인지 구분된다. 이걸 구분하지 못하면 멀쩡한 앱을 고치기 시작한다.
26. **서버 CPU 사용률의 분모를 머신 코어 수로 두지 말 것.** 제안서 §5.2의 "부하 30/65/90% CPU"는 **할당 CPU 대비** 사용률이다(Fargate 태스크의 vCPU). 머신 전체 코어로 나누면 단일 스레드 Node 프로세스는 18코어 머신에서 상한이 5.6%가 되어 **30%조차 원리적으로 도달 불가능**하고, 캘리브레이션이 VU를 끝까지 올리다 실패한다. 분모(`allocatedCores`)를 스냅샷과 캘리브레이션 산출물에 함께 기록해야 한다 — 이 값이 다르면 "부하 65%"가 두 환경에서 다른 것을 뜻한다.
27. **부하 생성기에는 요청 사이 대기가 있어야 한다.** VU는 정수인데 대기가 없으면 VU 하나가 코어의 절반을 먹는다(실측 50.8%). 그러면 30/65/90% 어디에도 격자가 걸리지 않는다. 대기 1초를 넣으면 VU당 ~5%가 되어 세 수준을 모두 맞출 수 있고, 연속 요청보다 실제 트래픽에 가깝다. **k6 스크립트와 로컬 생성기가 같은 값을 써야** 캘리브레이션 결과가 옮겨진다.
28. **부하 CPU를 1초 델타의 중앙값으로 추정하지 말 것.** 대기가 1초인 환경에서는 1초 창에 요청이 0건이거나 2건이라 표본이 튀고, 중앙값을 취해도 VU에 대해 단조롭지 않은 곡선이 나온다(실측: VU 2→14.5%, 4→9.2%, 8→36.9%, 16→61.3%, 32→49.4%). 이진 탐색은 단조성을 가정하므로 그 위에서 헤맨다. **관측 구간 전체를 한 번의 델타**로 재면 정의상 그 구간의 평균 사용률이 되고 표본 분산이 사라진다.
29. **지수 탐색은 상한에서 반드시 한 번 재야 한다.** `while (hi <= MAX_VUS)`로 두면 `hi`가 상한을 넘는 순간 루프가 빠져나가면서 **마지막으로 잰 더 낮은 VU의 값을 상한의 값인 것처럼 보고한다.** 실측에서 "VU 96에서도 81.5%"라고 출력했는데 96은 측정한 적조차 없었다. 측정하지 않은 값이 셀 정의에 적히는 것이라 조용히 넘기면 안 된다.
30. **이상치를 모드별로 계산할 것.** 모드를 합쳐 MAD 트리밍을 하면 CSR(~2,340)과 나머지(~750)가 한 분포에 섞여 대량으로 잘린다. 그 수치는 데이터가 아니라 **집계 방식이 만든 것**이고, 그 위에서 한 비교는 전부 무효다(꼬리 진단 첫 시도가 이렇게 무너졌다).
31. **상호작용 시퀀스가 실제로 존재하는 위젯을 겨냥하는지 확인할 것.** 콘텐츠형은 위젯이 toc·share뿐인데 시퀀스가 `.note input`을 찾아 매 반복 3초 타임아웃을 버렸다. 측정 창이 유형마다 달라져 노이즈 노출 시간이 달라지고, 그것만으로 이상치 제거율이 24%까지 올라간다. 실패는 기록되고 있었으므로 조용한 오염은 아니었지만, **셀마다 측정 창 길이가 다른 것 자체가 통제 실패**다.
32. **Streaming SSR은 청크 도착 경계 때문에 구조적으로 산포가 크다.** LCP 요소가 실린 청크가 한 세그먼트에 담기느냐 두 번에 걸치느냐가 실행마다 갈리고, 그 차이가 정확히 RTT의 배수로 나타난다(실측: 최대 − 중앙값 = 301ms ≈ 2 × 150ms). 제어할 수 없으므로 그리드 축이 아니라 **공변량**으로 기록한다 — `responseEnd − responseStart`가 그 구간이다.
33. **`stale` 셀은 반복당 ~62초가 든다 — 감수하기로 했다.** 엔트리를 만든 뒤 revalidate 창(60초)이 지나야 하고, stale 응답 한 번이 백그라운드 재생성을 유발해 반복마다 다시 기다려야 한다. 짧은 revalidate를 가진 SSG 라우트 변형을 두는 대안은 `T`가 다른 라우트를 만들어 §3.1.2의 `missRate`가 라우트마다 다른 스케일을 갖게 한다 — 캐시 축을 도입한 목적이 그 항의 관측이므로 **측정 대상을 바꾸느니 시간을 쓴다.** 비용은 직렬 413시간, 라우트 10-way 병렬에서 ~41시간이다(아래 §12 참조). `--skip-stale`은 검증용 탈출구이지 수집 옵션이 아니다.
...
| 5 | 부하·측정 워커 — k6 캘리브레이션, Playwright, 상호작용 시퀀스 | 반복 30회의 분산이 모드 간 차이보다 작은가 | 완료 |

3단계에서 모든 유형이 같은 모드를 선호하면 §5의 파라미터를 재조정한다. 5단계 검증이 통과하지 못하면 **factorial 수집을 시작해선 안 된다.** 노이즈가 신호보다 크면 10,400셀을 모아도 결정 경계가 나오지 않는다.
...
조건: 기기 `low`(CPU 4×) · 네트워크 `3g-fast` · 부하 Idle · 반복 30회 · 워밍업 3회.
점수는 `LCP + 2·TBT`(순위 판정용 대리 지표).

| 유형 | 최우수 | pooled_sd | 폭 | 분산비 | 분해능 | 인접쌍 분리 |
|---|---|---|---|---|---|---|
| content | ssg 736 | 25.3 | 1606 | 0.016 | 1.81 | 1/4 |
| dashboard | ssr 838 | 27.2 | 1290 | 0.021 | 4.54 | 2/3 |
| form | ssr 685 | 21.7 | 1083 | 0.020 | 21.70 | 1/3 |

세 유형 모두 합격 기준(분산비 < 0.5)을 크게 여유 있게 통과했다. **반복 노이즈는 문제가
아니다** — pooled_sd가 21~27점인데 CSR과의 격차는 1000점을 넘는다.

동시에 세 유형 전부에서 **1·2위가 노이즈 안에 있다.** content는 ssg(736)와 islands(750)의
차이가 14점인데 pooled_sd가 25점이고, form은 ssr(685)·stream(686)·islands(688)이 3점 안에
모여 있다. 이는 측정 실패가 아니라 3단계에서 관측된 것과 같은 사실이고, **마진 폴백 τ가
필요한 이유의 실측 근거**다. 5단계가 확인한 것은 "그 결론을 내릴 만큼 반복 노이즈가
작다"는 것이다.

#### 두 번의 실패에서 배운 것

**첫 실패 — INP가 통째로 결측됐다.** 폼형만 INP가 전부 null이었는데, 하필 폼형의 지배 축이
INP다. 원인이 두 겹이었다: Playwright의 `fill()`·`selectOption()`은 값을 코드로 넣을 뿐이라
사용자 상호작용으로 집계되지 않고, 설령 집계돼도 `event` 타이밍은 durationThreshold 미만이면
엔트리가 전달되지 않는다(§11-22, §11-23).

**두 번째 실패 — 실행 중 네트워크가 끊겼다.** 값이 3~5배 부풀고 순위까지 뒤섞였다
(content에서 islands가 csr보다 나쁘게 나왔다). 같은 코드가 스모크 테스트와 재측정에서는
정상값을 냈으므로 코드가 아니라 환경이었다. 이 사건이 §11-24·§11-25를 낳았고,
검증 스크립트에 드리프트·스파이크 감지가 붙었다.

두 실패 모두 **검증이 통과시키지 않고 거부했다는 점**이 중요하다. 이 상태로 10,400셀 ×
30회를 수집했다면 312,000회를 버렸을 것이고, 그때는 이미 며칠이 지난 뒤다.

#### 부하 캘리브레이션

이 개발 머신(할당 코어 1)에서 목표 CPU에 도달하는 VU: `low` 6 (31.1%), `mid` 30 (62.8%),
`high` 71 (91.9%). 환경이 바뀌면 다시 산출해야 하는 값이고, `allocatedCores`·`machineCores`가
산출물에 함께 기록된다.

여기까지 오는 데 네 번의 실패가 필요했고, 넷 다 **부하를 셀 변수로 고정한다**는 요구가
실제로 성립하는지 검사하다 나왔다(§11-26, §11-28, §11-29). 이 결함들이 남아 있었다면
부하 축이 있는 셀 전체 — 그리드의 3/4 — 가 거짓 라벨을 달았을 것이다.

#### 콘텐츠형의 두꺼운 꼬리 — 원인 규명 (24% → 8%)

content만 이상치 제거율이 24%(36/150)였다. dashboard 3%, form 1%와 대비된다.
`workers/diagnose-tail.mjs`로 파고들어 두 가지를 확인했다.

**기각된 가설 — LCP 요소 전환.** content는 `<img>`를 가진 유일한 유형이라(히어로,
1200×630) LCP 후보가 이미지와 텍스트 사이에서 뒤집히면 이봉 분포가 된다고 보았다.
150개 표본 전부 `#sec-01>p`(첫 섹션 문단)였고, `리소스 로드 지연`·`로드 시간`이 전부 0이다
— 텍스트 요소는 별도 요청이 없다. **히어로 이미지는 한 번도 LCP가 아니었다.**
HTTP/1.1 연결 슬롯 경합 가설도 함께 무너진다.

**주된 원인 — 상호작용 시퀀스의 죽은 3초.** 콘텐츠형은 `interactiveCount`가 2라 위젯이
toc·share뿐인데 시퀀스가 `.note input`에 입력을 시도했다. note 위젯은 **존재한 적이 없어**
매 반복 3초짜리 `waitFor` 타임아웃을 버렸고, 그만큼 측정 창이 다른 유형보다 길어져
주변 노이즈에 더 오래 노출됐다. 이 단계가 실제 위젯을 쓰도록 고친 뒤 24% → 8%로 떨어졌다.

**남은 8%는 Streaming SSR에 몰려 있다.** 모드별로 보면 ssr 0/30, ssg 0/30, islands 2/30,
csr 2/30인데 **stream만 8/30**이다. 요소 렌더 지연도 stream만 산포가 크다
(중앙값 536ms, MAD 26.4, 최대 837ms — 다른 모드는 MAD 4~14, 최대가 중앙값의 +5%).

```
stream 렌더 지연 최대 − 중앙값 = 837 − 536 = 301ms ≈ 2 × RTT(150ms)
```

정확히 왕복 두 번이다. 스트리밍은 응답이 청크로 나뉘고 LCP 요소가 실린 청크가
**한 세그먼트에 담기느냐 두 번에 걸치느냐**가 실행마다 갈린다. 이는 측정 결함이 아니라
**Streaming SSR의 성질**이며, 통제되지 않은 축은 "청크 도착 경계"다.

수집에서는 이 축을 공변량으로 기록하는 편이 낫다 —
`PerformanceNavigationTiming`의 `responseEnd − responseStart`가 스트리밍 지속 구간이므로,
그 값을 함께 남기면 사후에 분리할 수 있다. 그리드 축으로 올릴 성질은 아니다(제어 불가능하다).

> **이상치는 반드시 모드별로 계산한다.** 첫 진단이 모드를 합쳐 계산해서 A/B 비교가 통째로
> 무효였다. CSR(~2,340)과 나머지(~750)를 한 분포로 묶으면 MAD 트리밍이 대량으로 잘라내고,
> 그 수치는 데이터가 아니라 집계 방식이 만든 것이 된다.

### 그리드 크기 — 캐시 축을 반영한 10,400셀

```
Σ_r |M(r)|                    = 110      행동 공간의 크기
4 × 5 × 4 × Σ_r(|M(r)| + 2·[ssg ∈ M(r)])
                              = 80 × 130 = 10,400   실제 측정량
```

SSG 가능 라우트가 10종(콘텐츠 5 + 목록 5)이고 각각 캐시 상태가 3개이므로, SSG 셀 10개가
30개로 늘어난다. 캐시 상태를 통제하지 않으면 한 셀 안에서 미스와 히트가 섞여 분산이
폭발하고, §3.1.2의 `missRate`를 **관측할 수 없게 된다** — 통제되지 않은 캐시 상태는
추정 대상이 아니라 잡음이 된다. 늘어난 1,600셀은 그 관측 가능성의 대가다.

제안서 §5.2의 초기 수식은 캐시 축을 표에 넣고도 조건 수에서 빠뜨려 `80 × 110 = 8,800`이었다.
워커의 `expandGrid()`가 캐시 축을 반영해 구현되어 있고, **제안서 쪽을 10,400으로 수정했다.**

두 수를 모두 실험 메타데이터에 기록한다. `Σ|M(r)|`은 행동 공간의 크기이고 총 셀 수는
측정량이라, 서로 다른 질문에 답한다.

#### `offline-first` 네트워크 수준 — 확정

제안서 §5.2의 5번째 네트워크 수준. "완전 오프라인"으로 읽으면 페이지가 뜨지 않아 측정이
성립하지 않는다. **"오프라인 우선 설계가 필요할 만큼 열악한 망"으로 확정한다** —
0.25Mbps, RTT 900ms의 극단적 저대역·고지연 프로파일이며, 다른 넷과 마찬가지로 네트워크
조건 축으로만 취급한다.

Service Worker 캐시 히트 상태를 축으로 넣는 해석은 채택하지 않는다. SUT에 Service Worker가
없고, 그 축은 계측을 새로 설계해야 하는 별도 실험이라 이 그리드의 범위 밖이다.

### stale 축의 실행 비용 — 감수한다 (결정됨)

```
stale 셀   800  = SSG 라우트 10 × 기기 4 × 네트워크 5 × 부하 4
stale 반복 24,000  (셀당 30회) × 반복당 ~62초

직렬                          413 시간 (17.2일)
라우트 10-way 병렬             41 시간
```

**병렬 상한이 10인 이유:** ISR 엔트리는 라우트 경로 단위로 공유되므로 같은 라우트의
stale 셀을 두 워커가 동시에 돌리면 서로의 stale 타이머를 리셋한다. 부하 수준은 SUT 전역
조건이라 수준끼리도 직렬이다. 따라서 병렬화 가능한 축은 SSG 라우트(10종)뿐이다.

**대안을 택하지 않은 이유:** 짧은 재검증 주기를 가진 SSG 라우트 변형을 두면 대기는 사라지지만
`T`가 다른 라우트가 생기고, §3.1.2의 `missRate = 1/max(1, rps_r·T)`가 라우트마다 다른 스케일을
갖게 된다. 캐시 축을 도입한 목적이 바로 그 항의 관측이므로 **측정 대상을 바꾸느니 시간을 쓴다.**

**더 줄이려면 인터리빙이 필요하다.** 현재 `run.mjs`는 셀을 순차 처리해서 stale 대기 62초 동안
아무것도 하지 않는다. 대기 중에 다른 셀을 측정하도록 바꾸면 대기 대부분을 숨길 수 있다 —
비-stale 측정이 288,000회(@5초 ≈ 400시간)이므로 흡수할 여지가 충분하다.
6단계에서 실행 시간이 문제가 되면 그때 손대면 되고, 지금 필요한 최적화는 아니다.
