...
CloudFormation 스택 한도는 500이다. 샤드당 5리소스라 90샤드까지는 한 스택에 들어간다.
역할(실행·태스크)을 샤드마다 만들지 않고 공유해서 나온 수다 — 나누면 샤드당 15리소스가
되어 32샤드에서 한도에 닿는다. 샤드는 같은 이미지로 같은 일을 하는 복제본이라 권한이
다를 이유가 없고, 격리해야 하는 SUT는 태스크 단위로 이미 갈라져 있다.
