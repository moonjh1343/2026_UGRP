/**
 * 배경 부하 — k6 스크립트 (제안서 §5.3 산출물, **현재 미배포**).
 *
 * 실제 배포는 `control.mjs`(Node 생성기)로 뜬다 — load/Dockerfile과 그 파일의
 * 첫 주석이 k6 대신 Node를 쓰는 근거다. 이 스크립트는 수만 VU가 필요해질 때를
 * 위한 대체 경로로만 남긴다. profile.json을 generator.mjs와 공유하는 한
 * 캘리브레이션된 VU 수는 어느 쪽에서도 같은 부하를 뜻한다.
 *
