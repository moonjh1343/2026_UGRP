...
| 6 factorial 수집 | 본수집 `grid-v1` 2026-08-14 시작(10,400셀·20샤드) — 8/17 01:26 72h 태스크 타임아웃으로 FAILED(98.9%), high 샤드 15·18(잔여 114셀)은 `infra/scripts/resume-shards.sh`로 8/17 14:55 재개. 데이터는 S3(`UGRP_RESULTS_BUCKET`)·체크포인트는 DynamoDB. `workers/runs/pilot-low-idle/`·slice-b2는 그 이전 파일럿(SSG 캐시 축은 revalidate no-op 버그로 의심 대상) |
